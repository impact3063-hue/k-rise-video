...
# -*- coding: utf-8 -*-
"""
TikTok Sandbox API 接続テストスクリプト
Sandbox環境でのAPI接続をテストします
"""
...
def test_environment_variables():
    ...
    if not TIKTOK_OPEN_ID:
        print("  ✗ TIKTOK_OPEN_ID が設定されていません")
        issues.append("TIKTOK_OPEN_ID が未設定")
    else:
        print(f"  ✓ TIKTOK_OPEN_ID: {TIKTOK_OPEN_ID}")
    
    print(f"\n  📍 API Base URL: {SANDBOX_API_BASE}")
    print(f"  🔧 環境: Sandbox")
...
def test_user_info():
    ...
    print_section("ユーザー情報取得テスト (Sandbox)")
    
    if not TIKTOK_ACCESS_TOKEN:
        print("  ✗ トークンが設定されていないためスキップ")
        return False, ["トークン未設定"]
    
    url = f"{SANDBOX_API_BASE}/user/info/"
    headers = {
...
def test_video_upload_permissions():
    ...
    print_section("動画アップロード権限テスト (Sandbox)")
    ...
    url = f"{SANDBOX_API_BASE}/post/publish/video/init/"
    headers = {
        'Authorization': f'Bearer {TIKTOK_ACCESS_TOKEN}',
...
def print_troubleshooting_guide(all_issues):
    ...
    if not all_issues:
        print("\n  ✓ すべてのテストに合格しました！")
        print("  TikTok Sandbox APIへの接続は正常です。")
        return
    ...
    if any('トークン未設定' in issue or 'プレースホルダー' in issue for issue in all_issues):
        print("\n  📝 Sandboxトークンを取得する必要があります:")
        print("     1. Sandbox認証URLを生成:")
        print("        python generate_auth_url_sandbox.py")
        print("     2. ブラウザで認証URLを開く")
    ...
    if any('無効' in issue or '期限切れ' in issue for issue in all_issues):
        print("\n  🔄 トークンを再取得する必要があります:")
        print("     1. python generate_auth_url_sandbox.py")
        print("     2. 新しい認証コードを取得")
        print("     3. python get_token_sandbox.py")
        print("     4. 取得したトークンを .env に設定")
    
    if any('権限' in issue for issue in all_issues):
        print("\n  🔐 アプリの権限を確認してください:")
        print("     1. TikTok Developer Portal でアプリを開く")
        print("     2. 'Products' セクションを確認")
        print("     3. 以下のProductが追加されているか確認:")
        print("        - Login Kit")
        print("        - Content Posting API")
        print("     4. 権限を追加した場合は、トークンを再取得")
    
    print("\n  📚 詳細なガイド:")
    print("     TIKTOK_SANDBOX_AUTH_FIX.md を参照してください")

def main():
    """Main test function"""
    print_header("TikTok Sandbox API 接続テスト")
