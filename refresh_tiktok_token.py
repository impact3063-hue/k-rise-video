...
Helps you get a new access token for TikTok API
"""
...
def interactive_mode():
    ...
    print_header("TikTok アクセストークン更新ツール")
    
    print("\nこのスクリプトは、TikTok APIの新しいアクセストークンを取得します。")
    ...
    if success:
        ...
        print("  2. python upload_tiktok_auto.py で動画をアップロード")
        return True
    else:
        return False

def quick_mode():
    """Quick mode - just exchange a code for token"""
    print_header("TikTok トークン取得 (クイックモード)")
