/**
 * 🎬 K-RISE TikTok Video 5 - CapCut Style Phrase Display
 * CapCut風フレーズ表示：音声に完全連動する短いテロップ
 * 
 * データソース: video-data-capcut-style.json
 * 
 * 特徴:
 * - 長文固定表示を撤廃
 * - 音声の発音タイミングに合わせて短いフレーズ（2〜5文字）を表示
 * - 1.1倍ポップインアニメーション
 * - フェードイン/アウト効果
 * - ゴールドグロー強調
 */
 ...
 * メインコンポジション
 */
export const KRiseTikTok5: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps, durationInFrames } = useVideoConfig();

  // 現在表示すべきフレーズを取得
  const currentPhrase = useMemo(() => {
    return videoData.phrases.find(
      (phrase: Phrase) => frame >= phrase.startFrame && frame < phrase.endFrame
    );
  }, [frame]);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#000" }}>
      ...
      >
        🎯 K-RISE CapCut Style | Frame: {frame} | v{videoData.version}
      </div>
    </AbsoluteFill>
  );
};
